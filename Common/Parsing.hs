...
psqlDump :: Parser ([String], [[String]])
psqlDump = do
  cols <- sepBy (many (noneOf ",\n\"'|+")) (oneOf "|+")
  sepBy (many $ char '-') (char '+')
  cells <- endBy psqlLine eol
  return (cols, cells)
psqlLine = sepBy (many $ noneOf ",\n") (char '|')
nameFile = sepBy nameCell (char ',')
lineS :: Parser [Integer]
lineS = sepBy icell (char ' ')
line = sepBy icell (char ',')
icell :: Parser Integer
icell = read <$> many1 digit
